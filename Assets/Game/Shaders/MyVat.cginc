#ifndef MY_GA_FUQUNA_VATBAKER_VAT_INCLUDED
#define MY_GA_FUQUNA_VATBAKER_VAT_INCLUDED
...
{
    return fmod(time, _VatAnimLength) * _VatAnimFps;
}
...
{
    return tex2Dlod(_VatPositionTex, float4(CalcVatTexCoord(vertexId, animationTime).xy, 0, 0)).xyz;
}

inline float3 GetVatNormal(uint vertexId, float animationTime)
{
    return tex2Dlod(_VatNormalTex, float4(CalcVatTexCoord(vertexId, animationTime).xy, 0, 0)).xyz;
}
